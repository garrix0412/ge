"""
LSTM model with dual-head (regression + classification) output.
...
class _LSTMNetwork(nn.Module):
    """LSTM encoder with dual regression / classification heads.
    ...
    dropout : float
        Dropout rate applied between LSTM layers (>= 2 layers).
    horizon : int
    ...
    def __init__(
        ...
        self.reg_head = nn.Linear(fc_input, horizon)

        # Classification head — predicts probability of price going up
        self.cls_head = nn.Sequential(
            nn.Linear(fc_input, horizon),
            nn.Sigmoid(),
        )
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        last_hidden = lstm_out[:, -1, :]  # (batch, hidden*directions)
...
class LSTMModel(BaseTorchModel):
    """LSTM-based forecaster with dual regression / classification heads.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.hidden_size: int = lstm_cfg.hidden_size
        self.num_layers: int = lstm_cfg.num_layers
        self.dropout: float = lstm_cfg.dropout
        self.bidirectional: bool = lstm_cfg.bidirectional
    ...
    def fit(
        ...
        **kwargs: Any,
        ...
        """Train the LSTM with a dual-head loss.
        ...
        logger.info("Starting LSTM training — %d epochs max, patience=%d, device=%s",
        ...
        self.model.eval()
        logger.info("LSTM training complete — %d epochs, best_val_loss=%.6f",
